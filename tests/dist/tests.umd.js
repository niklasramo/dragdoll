    ...
    describe('BaseSensor', () => {
        ...
        describe('_start method', () => {
            it(`should create drag data`, function () {
                ...
                chai.assert.deepEqual(s.drag, { x: 1, y: 2 });
                s.destroy();
            });
            ...
            it(`should emit "start" event with correct arguments after updating instance properties`, function () {
                ...
                let emitCount = 0;
                s.on('start', (data) => {
                    chai.assert.deepEqual(s.drag, { x: data.x, y: data.y });
                    chai.assert.equal(s.isDestroyed, false);
            ...
            it(`should not do anything if drag is already active`, function () {
                ...
                chai.assert.deepEqual(s.drag, drag);
        ...
        describe('_move method', () => {
            it(`should update drag data to reflect the provided coordinates`, function () {
                ...
                chai.assert.deepEqual(s.drag, { x: 3, y: 4 });
                s.destroy();
            });
            it(`should not modify isDestroyed property`, function () {
                const s = new BaseSensor();
       ...
       `, function () {
                ...
                s.on('cancel', (data) => {
                    chai.assert.deepEqual(s.drag, { x: startArgs.x, y: startArgs.y });
    ...
    describe('PointerSensor', () => {
        ...
        describe('target element parameter', () => {
            it('should accept document.documentElement', function () {
                ...
                chai.assert.notEqual(s.drag, null);
                s.destroy();
            });
        ...
        describe('sourceEvents option', () => {
            it('should listen to mouse/pointer/touch events when set to "mouse"/"pointer"/"touch"', function () {
                ...
                chai.assert.deepEqual(mouseList, ['start', 'move', 'end']);
                chai.assert.deepEqual(pointerList, []);
                chai.assert.deepEqual(touchList, []);
                mouseList.length = 0;
                createFakeDrag([
                    { x: 1, y: 1 },
                    { x: 2, y: 2 },
                ...
                chai.assert.deepEqual(mouseList, []);
                chai.assert.deepEqual(pointerList, ['start', 'move', 'end']);
                chai.assert.deepEqual(touchList, []);
                ...
                chai.assert.deepEqual(mouseList, []);
                chai.assert.deepEqual(pointerList, []);
                chai.assert.deepEqual(touchList, ['start', 'move', 'end']);
                mouseSensor.destroy();
        ...
        describe('startPredicate option', () => {
            ...
            it('should allow start when true is returned and prevent start when false is returned', function () {
                ...
                document.body.dispatchEvent(new MouseEvent('mousedown'));
                chai.assert.notEqual(s1.drag, null);
                chai.assert.equal(s2.drag, null);
                s1.destroy();
                s2.destroy();
            });
        });
            ...
            it(`should be triggered correctly on mousedown`, function () {
                ...
                s.on('start', (e) => {
                    if (startEvent === null) {
                        ...
                        chai.assert.fail('start event listener called twice');
                    }
                });
                createFakeDrag([
                    { x: 1, y: 1 },
                ...
                chai.assert.deepEqual(startEvent, {
                    type: 'start',
            ...
            it(`should be triggered correctly on mousemove`, function () {
                ...
                s.on('move', (e) => {
                    if (moveEvent === null) {
                        ...
                        chai.assert.fail('move event listener called twice');
                    }
                });
                createFakeDrag([
                    { x: 1, y: 1 },
                ...
                chai.assert.deepEqual(moveEvent, {
                    type: 'move',
                    srcEvent: sourceEvent,
                    target: el,
                    pointerId: -1,
            ...
            it(`should be triggered correctly on mouseup`, function () {
                ...
                s.on('end', (e) => {
                    if (endEvent === null) {
                        ...
                        chai.assert.fail('end event listener called twice');
                    }
                });
                createFakeDrag([
                    { x: 1, y: 1 },
                ...
                chai.assert.deepEqual(endEvent, {
                    type: 'end',
                    srcEvent: sourceEvent,
                    target: el,
                    pointerId: -1,
            ...
            it(`should be triggered correctly on pointercancel`, function () {
                ...
                s.on('cancel', (e) => {
                    if (cancelEvent === null) {
                        ...
                        chai.assert.fail('cancel event listener called twice');
                    }
                });
                createFakeDrag([
                    { x: 1, y: 1 },
                ...
                chai.assert.deepEqual(cancelEvent, {
                    type: 'cancel',
                    srcEvent: sourceEvent,
                    target: el,
                    pointerId: sourceEvent.pointerId,
