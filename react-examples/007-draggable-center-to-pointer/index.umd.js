...
//#endregion
//#region react-examples/007-draggable-center-to-pointer/index.tsx
...
const DraggableCardMemo = (0, import_react.memo)(function DraggableCard() {
	...
	const draggableSettings = (0, import_react.useMemo)(() => ({
		...
		positionModifiers: [(change, { drag: drag$1, item, phase }) => {
			if (phase === "start" && drag$1.sensor instanceof d && drag$1.items[0].element === item.element) {
				const { clientRect } = item;
				const { x: x$1, y: y$1 } = drag$1.startEvent;
				const targetX = clientRect.x + clientRect.width / 2;
				const targetY = clientRect.y + clientRect.height / 2;
				change.x = x$1 - targetX;
				change.y = y$1 - targetY;
			}
			return change;
		}]
	}), []);
