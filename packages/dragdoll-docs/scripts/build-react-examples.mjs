...
async function buildExampleDirectories() {
  const examplesDir = path.join(process.cwd(), './react-examples/');
  ...
  for (const dir of directories) {
    ...
    const outputDir = path.join(process.cwd(), `./docs/public/react-examples/${exampleName}`);
...
async function buildExamplesMarkdown() {
  const examplesDir = path.join(process.cwd(), './react-examples/');
  const markdownFilePath = path.join(process.cwd(), './docs/react/examples.md');
  ...
  for (const dir of directories) {
    ...
    try {
      ...
      markdownContent += `<iframe src="/dragdoll/react-examples/${exampleName}/index.html"></iframe>`;
      markdownContent += `<a class="example-link" target="_blank" href="/dragdoll/react-examples/${exampleName}/index.html" title="Open in a new tab">`;
