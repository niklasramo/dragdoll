(function(factory) {
	...
	var E = class {
		...
		constructor(e, t) {
			...
			let i = t$3(e), a = e.getBoundingClientRect(), s = r$2(e, !0);
	...
	function t(t, n, r, i) {
		...
		let s = c(a), c$2 = s.m11, l = s.m12, u = s.m21, d = s.m22, f = t.getBoundingClientRect(), p = n - f.left, m = r - f.top, h = c$2 * d - l * u;
		if (Math.abs(h) < 1e-10) return o.x = p, o.y = m, o;
		let g = 1 / h;
		return o.x = (d * p - u * m) * g, o.y = (-l * p + c$2 * m) * g, o;
	}
	//#endregion
	//#region examples/core/013-draggable-multi-drag-preview/index.ts
	const cards = Array.from(document.querySelectorAll(".card.draggable"));
	new R([new d(window, { startPredicate: (e) => {
		if ("button" in e && e.button > 0) return false;
		const target = e.target;
		if (!target) return false;
		return cards.some((card) => card.contains(target));
	} }), new a(null, { startPredicate: () => {
		...
		elements: () => {
			...
			for (let i = 0; i < clones.length; i++) {
				...
				t(clone, cardRect.x, cardRect.y, offset);
			}
			for (let i = 0; i < clones.length; i++) {
				const offset = cloneOffsets[i];
				if (!offset) continue;
