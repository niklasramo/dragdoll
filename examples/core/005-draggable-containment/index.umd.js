(function(factory) {
	...
	function c$1(n) {
		return n === `auto` || n === void 0 ? n$4 ? `pointer` : t$1 ? `touch` : `mouse` : n;
	}
	const l = {
		pointer: {
			start: `pointerdown`,
	...
	var d = class {
		...
		_onMove(e$8) {
			...
			t.deltaX = a - t.x, t.deltaY = o - t.y, t.x = a, t.y = o, r.type = e.Move, r.srcEvent = e$8, r.target = i.target, r.x = a, r.y = o, r.deltaX = t.deltaX, r.deltaY = t.deltaY, this._emitter.emit(r.type, r);
		}
		_onCancel(e$9) {
			let t = this.drag, r = this._eventData;
			if (!t || !r) return;
			...
			if (!i) return;
			let a = i.clientX, o = i.clientY;
			t.deltaX = a - t.x, t.deltaY = o - t.y, t.x = a, t.y = o, r.type = e.Cancel, r.srcEvent = e$9, r.target = i.target, r.x = a, r.y = o, r.deltaX = t.deltaX, r.deltaY = t.deltaY, this._emitter.emit(r.type, r), this._resetDrag();
		}
		_onEnd(e$10) {
			let t = this.drag, r = this._eventData;
			if (!t || !r) return;
			...
			if (!i) return;
			let a = i.clientX, o = i.clientY;
			t.deltaX = a - t.x, t.deltaY = o - t.y, t.x = a, t.y = o, r.type = e.End, r.srcEvent = e$10, r.target = i.target, r.x = a, r.y = o, r.deltaX = t.deltaX, r.deltaY = t.deltaY, this._emitter.emit(r.type, r), this._resetDrag();
		}
		...
		updateSettings(e) {
			...
			let { listenerOptions: t, sourceEvents: n, startPredicate: r, cancelOnVisibilityChange: i, cancelOnEscape: a, preventNativeDrag: o, preventContextMenu: u } = e, d = c$1(n), f = s$2(t);
	...
	function r$3(r, i = !1) {
		...
		if (a && a !== `none`) {
			...
			e.includes(`%`) && (e = `${parseFloat(e) / 100 * n$3(r)}px`), i.includes(`%`) && (i = `${parseFloat(i) / 100 * t(r)}px`), o ? l += `translate3d(${e},${i},${o})` : l += `translate(${e},${i})`;
		}
		if (o && o !== `none`) {
			let e = o.split(` `);
	...
	var E = class {
		...
		constructor(e, t) {
			...
			let i = t$2(e), a = e.getBoundingClientRect(), s = r$3(e, !0);
		...
		_updateContainerMatrices() {
			[this.elementContainer, this.dragContainer].forEach((e) => {
				if (!this._matrixCache.isValid(e)) {
					...
					c(e, n), r$2(r).multiplySelf(n).invertSelf(), this._matrixCache.set(e, t);
				}
			});
		}
	...
	var R = class {
		...
		_bindSensor(e$5) {
			this._sensorData.set(e$5, {
				...
				predicateEvent: null,
				onMove: (t) => this._onMove(t, e$5),
				onEnd: (t) => this._onEnd(t, e$5)
			});
			let { onMove: t, onEnd: n } = this._sensorData.get(e$5);
			e$5.on(e.Start, t, t), e$5.on(e.Move, t, t), e$5.on(e.Cancel, n, n), e$5.on(e.End, n, n);
		}
		_unbindSensor(e$6) {
			let t = this._sensorData.get(e$6);
			if (!t) return;
			let { onMove: n, onEnd: r } = t;
			e$6.off(e.Start, n), e$6.off(e.Move, n), e$6.off(e.Cancel, r), e$6.off(e.End, r), this._sensorData.delete(e$6);
		}
		...
		_onMove(n, r) {
			...
			if (i) switch (i.predicateState) {
				...
				case M.Resolved:
					this.drag && (Object.assign(this.drag.moveEvent, n), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareMove(), this._applyMove()) : (r$4.once(n$5.read, this._prepareMove, this._moveId), r$4.once(n$5.write, this._applyMove, this._moveId)));
					break;
			}
		}
		_onScroll() {
		...
		resolveStartPredicate(n, r) {
			...
			}), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareStart(), this._applyStart()) : (r$4.once(n$5.read, this._prepareStart, this._startId), r$4.once(n$5.write, this._applyStart, this._startId)));
		}
		...
		stop() {
			...
			if (n.isEnded = !0, this._prepareStart(), this._applyStart(), this._startPhase = j.None, r$4.off(n$5.read, this._startId), r$4.off(n$5.write, this._startId), r$4.off(n$5.read, this._moveId), r$4.off(n$5.write, this._moveId), r$4.off(n$5.read, this._alignId), r$4.off(n$5.write, this._alignId), window.removeEventListener(`scroll`, this._onScroll, D), this._selectionChangeHandler &&= ((n.items[0]?.element?.ownerDocument ?? document).removeEventListener(`selectionchange`, this._selectionChangeHandler), null), this._pointerCaptureTarget && this._pointerCaptureId !== null) {
				try {
		...
		align(n = !1) {
			!this.drag || this.drag.isEnded || (n || this.settings.sensorProcessingMode === P.Immediate ? (this._prepareAlign(), this._applyAlign()) : (r$4.once(n$5.read, this._prepareAlign, this._alignId), r$4.once(n$5.write, this._applyAlign, this._alignId)));
		}
		getClientRect() {
	...
	function s(s, c) {
		let l = c?.trackSensorDrift ?? (({ drag: e }) => e.sensor instanceof d), u = c?.snapX || 0, d$1 = c?.snapY || 0;
		return function(t, c) {
			let f = e$3(s(c), n$2), p = e$3(c.item.clientRect, r$1), m = c.item.data, h = m.__containment__ || {
				drift: {
					x: 0,
					y: 0
				},
			...
			if (m.__containment__ ||= (h.trackDrift = typeof l == `function` ? l(c) : l, h), c.phase === `start`) return t.x &&= (a$1(p.left, p.right, f.left, f.right, 0, t.x, !1), i$2.change), t.y &&= (a$1(p.top, p.bottom, f.top, f.bottom, 0, t.y, !1), i$2.change), (u || d$1) && (h.startLeft = p.left + t.x, h.startTop = p.top + t.y, h.startRight = p.right + t.x, h.startBottom = p.bottom + t.y), t;
			if (u) {
				h.sensorX += t.x;
				let e = o(u, h.snapX, h.sensorX), n = h.snapX + e, r = Math.ceil((f.left - h.startLeft) / u) * u, i = Math.floor((f.right - h.startRight) / u) * u;
			...
			if (d$1) {
				h.sensorY += t.y;
				let e = o(d$1, h.snapY, h.sensorY), n = h.snapY + e, r = Math.ceil((f.top - h.startTop) / d$1) * d$1, i = Math.floor((f.bottom - h.startBottom) / d$1) * d$1;
	...
	var i$1 = class extends n$1 {
		...
		_start(n) {
			this.isDestroyed || this.drag || (super._start(n), r$4.on(n$5.read, this._tick, this._tick));
		}
		_end(n) {
			this.drag && (r$4.off(n$5.read, this._tick), super._end(n));
		}
		_cancel(n) {
			this.drag && (r$4.off(n$5.read, this._tick), super._cancel(n));
		}
		_tick(e) {
	...
	//#endregion
	//#region examples/core/005-draggable-containment/index.ts
