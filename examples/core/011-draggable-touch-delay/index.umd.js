(function(factory) {
	...
	const e$1 = typeof window < `u` && window.document !== void 0, t$1 = e$1 && `ontouchstart` in window, n$1 = e$1 && !!window.PointerEvent;
	...
	function c$2(n) {
		return n === `auto` || n === void 0 ? n$1 ? `pointer` : t$1 ? `touch` : `mouse` : n;
	}
	const l$1 = {
		pointer: {
			start: `pointerdown`,
	...
	var d = class {
		...
		_onMove(e$14) {
			...
			t.deltaX = a - t.x, t.deltaY = o - t.y, t.x = a, t.y = o, r.type = e.Move, r.srcEvent = e$14, r.target = i.target, r.x = a, r.y = o, r.deltaX = t.deltaX, r.deltaY = t.deltaY, this._emitter.emit(r.type, r);
		}
		_onCancel(e$15) {
			let t = this.drag, r = this._eventData;
			if (!t || !r) return;
			...
			if (!i) return;
			let a = i.clientX, o = i.clientY;
			t.deltaX = a - t.x, t.deltaY = o - t.y, t.x = a, t.y = o, r.type = e.Cancel, r.srcEvent = e$15, r.target = i.target, r.x = a, r.y = o, r.deltaX = t.deltaX, r.deltaY = t.deltaY, this._emitter.emit(r.type, r), this._resetDrag();
		}
		_onEnd(e$16) {
			let t = this.drag, r = this._eventData;
			if (!t || !r) return;
			let i = this._getTrackedPointerEventData(e$16);
			if (!i) return;
			let a = i.clientX, o = i.clientY;
			t.deltaX = a - t.x, t.deltaY = o - t.y, t.x = a, t.y = o, r.type = e.End, r.srcEvent = e$16, r.target = i.target, r.x = a, r.y = o, r.deltaX = t.deltaX, r.deltaY = t.deltaY, this._emitter.emit(r.type, r), this._resetDrag();
		}
	...
	function r$1(r, i = !1) {
		...
		if (a && a !== `none`) {
			...
			e.includes(`%`) && (e = `${parseFloat(e) / 100 * n(r)}px`), i.includes(`%`) && (i = `${parseFloat(i) / 100 * t(r)}px`), o ? l += `translate3d(${e},${i},${o})` : l += `translate(${e},${i})`;
		}
		if (o && o !== `none`) {
			let e = o.split(` `);
	...
	const a$1 = e$1 ? new DOMMatrix() : null, o$1 = e$1 ? new DOMMatrix() : null, s$1 = {
		x: 0,
		y: 0,
		z: 0
	};
	function c$1(t, c = new DOMMatrix()) {
		...
		for (r(c); l;) {
			...
			if (t && (a$1.setMatrixValue(t), !a$1.isIdentity)) {
				...
				i(t, s$1);
				let { x: n, y: u, z: d } = s$1;
				d === 0 ? (a$1.translateSelf(-n, -u), r(o$1).translateSelf(n, u)) : (a$1.translateSelf(-n, -u, -d), r(o$1).translateSelf(n, u, d)), a$1.preMultiplySelf(o$1), c.preMultiplySelf(a$1);
			}
			l = l.parentElement;
		}
		return c;
	...
	var E = class {
		...
		constructor(e, t) {
			...
			let i$2 = t$2(e), a = e.getBoundingClientRect(), s = r$1(e, !0);
		...
		_updateContainerMatrices() {
			[this.elementContainer, this.dragContainer].forEach((e) => {
				if (!this._matrixCache.isValid(e)) {
					let t = this._matrixCache.get(e) || [new DOMMatrix(), new DOMMatrix()], [n, r$3] = t;
					c$1(e, n), r(r$3).multiplySelf(n).invertSelf(), this._matrixCache.set(e, t);
				}
			});
		}
	...
	const N = {
		...
		applyPosition: ({ item: e, phase: t }) => {
			let n = t === F.End || t === F.EndAlign, [r$4, i] = e.getContainerMatrix(), [a, o] = e.getDragContainerMatrix(), { position: c, alignmentOffset: l, containerOffset: u, elementTransformMatrix: d, elementTransformOrigin: f, elementOffsetMatrix: p } = e, { x: m, y: h, z: g } = f, _ = !d.isIdentity && (m !== 0 || h !== 0 || g !== 0), v = c.x + l.x + u.x, y = c.y + l.y + u.y;
			r(k), _ && (g === 0 ? k.translateSelf(-m, -h) : k.translateSelf(-m, -h, -g)), n ? i.isIdentity || k.multiplySelf(i) : o.isIdentity || k.multiplySelf(o), r(A).translateSelf(v, y), k.multiplySelf(A), r$4.isIdentity || k.multiplySelf(r$4), _ && (r(A).translateSelf(m, h, g), k.multiplySelf(A)), d.isIdentity || k.multiplySelf(d), p.isIdentity || k.preMultiplySelf(p), e.element.style.transform = `${k}`;
		},
	...
	var R = class {
		...
		_bindSensor(e$11) {
			this._sensorData.set(e$11, {
				...
				predicateEvent: null,
				onMove: (t) => this._onMove(t, e$11),
				onEnd: (t) => this._onEnd(t, e$11)
			});
			let { onMove: t, onEnd: n } = this._sensorData.get(e$11);
			e$11.on(e.Start, t, t), e$11.on(e.Move, t, t), e$11.on(e.Cancel, n, n), e$11.on(e.End, n, n);
		}
		_unbindSensor(e$12) {
			let t = this._sensorData.get(e$12);
			if (!t) return;
			let { onMove: n, onEnd: r } = t;
			e$12.off(e.Start, n), e$12.off(e.Move, n), e$12.off(e.Cancel, r), e$12.off(e.End, r), this._sensorData.delete(e$12);
		}
		...
		_onMove(n, r) {
			...
			if (i) switch (i.predicateState) {
				...
				case M.Resolved:
					this.drag && (Object.assign(this.drag.moveEvent, n), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareMove(), this._applyMove()) : (r$2.once(n$2.read, this._prepareMove, this._moveId), r$2.once(n$2.write, this._applyMove, this._moveId)));
					break;
			}
		}
		_onScroll() {
		...
		resolveStartPredicate(n, r) {
			...
			}), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareStart(), this._applyStart()) : (r$2.once(n$2.read, this._prepareStart, this._startId), r$2.once(n$2.write, this._applyStart, this._startId)));
		}
		...
		stop() {
			...
			if (n.isEnded = !0, this._prepareStart(), this._applyStart(), this._startPhase = j.None, r$2.off(n$2.read, this._startId), r$2.off(n$2.write, this._startId), r$2.off(n$2.read, this._moveId), r$2.off(n$2.write, this._moveId), r$2.off(n$2.read, this._alignId), r$2.off(n$2.write, this._alignId), window.removeEventListener(`scroll`, this._onScroll, D), this._selectionChangeHandler &&= ((n.items[0]?.element?.ownerDocument ?? document).removeEventListener(`selectionchange`, this._selectionChangeHandler), null), this._pointerCaptureTarget && this._pointerCaptureId !== null) {
				try {
		...
		align(n = !1) {
			!this.drag || this.drag.isEnded || (n || this.settings.sensorProcessingMode === P.Immediate ? (this._prepareAlign(), this._applyAlign()) : (r$2.once(n$2.read, this._prepareAlign, this._alignId), r$2.once(n$2.write, this._applyAlign, this._alignId)));
		}
		getClientRect() {
	...
	function o(t) {
		let n = t$2(t);
		return !!(a.has(n.overflowY) || a.has(n.overflowX));
	}
	function s(e, n = []) {
		let r = e?.parentNode;
		for (n.length = 0; r && !e$2(r);) r instanceof Element ? (o(r) && n.push(r), r = r.parentNode) : r = r instanceof ShadowRoot ? r.host : r.parentNode;
		return n.push(window), n;
	}
	function c(e) {
		let t = [];
		return o(e) && t.push(e), s(e, t), t;
	}
	function l(e$7 = {}) {
		let t, a = 0, o = null, s, { touchDelay: l = 250, fallback: u = () => !0 } = e$7, d$1 = (e) => e.preventDefault(), f = (e) => {
			if (a) {
				if (t) {
		...
		return (e$6) => {
			if (!(e$6.sensor instanceof d)) return u(e$6);
			let { draggable: p, sensor: m, event: h } = e$6, g = h;
			if (g.pointerType === `touch`) {
				if (g.type === e.Start && (g.srcEvent.type === `pointerdown` || g.srcEvent.type === `touchstart`)) {
					o = g.target;
					let e$9 = o ? c(o) : [];
					e$9.forEach((e) => {
					...
					let u = () => {
						a && (p.off(I.End, u), p.sensors.forEach((e$8) => {
							e$8 instanceof d && e$8.off(e.End, u);
						}), o?.removeEventListener(`contextmenu`, d$1), e$9.forEach((e) => {
							e.removeEventListener(`touchmove`, f, { capture: !0 });
						}), a = 0, t = void 0, o = null, s = void window.clearTimeout(s));
					};
					t = void 0, a = g.srcEvent.timeStamp, o?.addEventListener(`contextmenu`, d$1), p.on(I.End, u), p.sensors.forEach((e$10) => {
						e$10 instanceof d && e$10.on(e.End, u);
			...
			return g.type === e.Start && !g.srcEvent.button;
		};
	}
	//#endregion
	...
	//#endregion
	//#region examples/core/011-draggable-touch-delay/index.ts
