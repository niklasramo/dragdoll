(function(factory) {
	...
	const e$1 = typeof window < `u` && window.document !== void 0, t$1 = e$1 && `ontouchstart` in window, n$2 = e$1 && !!window.PointerEvent;
	...
	function c$1(n) {
		return n === `auto` || n === void 0 ? n$2 ? `pointer` : t$1 ? `touch` : `mouse` : n;
	}
	const l = {
		pointer: {
			start: `pointerdown`,
	...
	function r$2(r, i = !1) {
		...
		if (a && a !== `none`) {
			...
			e.includes(`%`) && (e = `${parseFloat(e) / 100 * n$1(r)}px`), i.includes(`%`) && (i = `${parseFloat(i) / 100 * t(r)}px`), o ? l += `translate3d(${e},${i},${o})` : l += `translate(${e},${i})`;
		}
		if (o && o !== `none`) {
			let e = o.split(` `);
	...
	function c(t, c = new DOMMatrix()) {
		...
		for (r$1(c); l;) {
			...
			if (t && (a$1.setMatrixValue(t), !a$1.isIdentity)) {
				...
				i$1(t, s$1);
				let { x: n, y: u, z: d } = s$1;
				d === 0 ? (a$1.translateSelf(-n, -u), r$1(o$1).translateSelf(n, u)) : (a$1.translateSelf(-n, -u, -d), r$1(o$1).translateSelf(n, u, d)), a$1.preMultiplySelf(o$1), c.preMultiplySelf(a$1);
			}
			l = l.parentElement;
		}
		return c;
	...
	var R = class {
		...
		_onMove(n, r) {
			...
			if (i) switch (i.predicateState) {
				...
				case M.Resolved:
					this.drag && (Object.assign(this.drag.moveEvent, n), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareMove(), this._applyMove()) : (r$3.once(n$3.read, this._prepareMove, this._moveId), r$3.once(n$3.write, this._applyMove, this._moveId)));
					break;
			}
		}
		_onScroll() {
		...
		resolveStartPredicate(n, r) {
			...
			}), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareStart(), this._applyStart()) : (r$3.once(n$3.read, this._prepareStart, this._startId), r$3.once(n$3.write, this._applyStart, this._startId)));
		}
		...
		stop() {
			...
			if (n.isEnded = !0, this._prepareStart(), this._applyStart(), this._startPhase = j.None, r$3.off(n$3.read, this._startId), r$3.off(n$3.write, this._startId), r$3.off(n$3.read, this._moveId), r$3.off(n$3.write, this._moveId), r$3.off(n$3.read, this._alignId), r$3.off(n$3.write, this._alignId), window.removeEventListener(`scroll`, this._onScroll, D), this._selectionChangeHandler &&= ((n.items[0]?.element?.ownerDocument ?? document).removeEventListener(`selectionchange`, this._selectionChangeHandler), null), this._pointerCaptureTarget && this._pointerCaptureId !== null) {
				try {
		...
		align(n = !1) {
			!this.drag || this.drag.isEnded || (n || this.settings.sensorProcessingMode === P.Immediate ? (this._prepareAlign(), this._applyAlign()) : (r$3.once(n$3.read, this._prepareAlign, this._alignId), r$3.once(n$3.write, this._applyAlign, this._alignId)));
		}
		getClientRect() {
	...
	function s(s, c) {
		...
		return function(t, c) {
			let f = e$3(s(c), n), p = e$3(c.item.clientRect, r), m = c.item.data, h = m.__containment__ || {
				drift: {
					x: 0,
					y: 0
				},
			...
			if (m.__containment__ ||= (h.trackDrift = typeof l == `function` ? l(c) : l, h), c.phase === `start`) return t.x &&= (a(p.left, p.right, f.left, f.right, 0, t.x, !1), i.change), t.y &&= (a(p.top, p.bottom, f.top, f.bottom, 0, t.y, !1), i.change), (u || d$1) && (h.startLeft = p.left + t.x, h.startTop = p.top + t.y, h.startRight = p.right + t.x, h.startBottom = p.bottom + t.y), t;
			if (u) {
				h.sensorX += t.x;
			...
			} else t.x &&= (a(p.left, p.right, f.left, f.right, h.drift.x, t.x, h.trackDrift), h.drift.x = i.drift, i.change);
			if (d$1) {
				h.sensorY += t.y;
			...
			} else t.y &&= (a(p.top, p.bottom, f.top, f.bottom, h.drift.y, t.y, h.trackDrift), h.drift.y = i.drift, i.change);
			return t;
		};
	}
	//#endregion
	//#region examples/core/006-draggable-combined-modifiers/index.ts
	const THRESHOLD = 5;
	const GRID = 40;
