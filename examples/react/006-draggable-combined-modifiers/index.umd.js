(function(factory) {
	...
	const e$6 = typeof window < `u` && window.document !== void 0, t$10 = e$6 && `ontouchstart` in window, n$9 = e$6 && !!window.PointerEvent;
	...
	function c$2(n) {
		return n === `auto` || n === void 0 ? n$9 ? `pointer` : t$10 ? `touch` : `mouse` : n;
	}
	const l = {
		pointer: {
			start: `pointerdown`,
	...
	var d = class {
		...
		updateSettings(e) {
			...
			let { listenerOptions: t, sourceEvents: n, startPredicate: r, cancelOnVisibilityChange: i, cancelOnEscape: a, preventNativeDrag: o, preventContextMenu: u } = e, d = c$2(n), f = s$3(t);
	...
	function r$5(r, i = !1) {
		...
		if (a && a !== `none`) {
			...
			e.includes(`%`) && (e = `${parseFloat(e) / 100 * n$8(r)}px`), i.includes(`%`) && (i = `${parseFloat(i) / 100 * t$9(r)}px`), o ? l += `translate3d(${e},${i},${o})` : l += `translate(${e},${i})`;
		}
		if (o && o !== `none`) {
			let e = o.split(` `);
	...
	const a$2 = e$6 ? new DOMMatrix() : null, o$2 = e$6 ? new DOMMatrix() : null, s$2 = {
		x: 0,
		y: 0,
		z: 0
	};
	function c$1(t, c = new DOMMatrix()) {
		...
		for (r$4(c); l;) {
			...
			if (t && (a$2.setMatrixValue(t), !a$2.isIdentity)) {
				...
				i$3(t, s$2);
				let { x: n, y: u, z: d } = s$2;
				d === 0 ? (a$2.translateSelf(-n, -u), r$4(o$2).translateSelf(n, u)) : (a$2.translateSelf(-n, -u, -d), r$4(o$2).translateSelf(n, u, d)), a$2.preMultiplySelf(o$2), c.preMultiplySelf(a$2);
			}
			l = l.parentElement;
		}
		return c;
		...
		_updateContainerMatrices() {
			[this.elementContainer, this.dragContainer].forEach((e) => {
				if (!this._matrixCache.isValid(e)) {
					...
					c$1(e, n), r$4(r).multiplySelf(n).invertSelf(), this._matrixCache.set(e, t);
				}
			});
		}
	...
	var R = class {
		...
		_onMove(n, r) {
			...
			if (i) switch (i.predicateState) {
				...
				case M.Resolved:
					this.drag && (Object.assign(this.drag.moveEvent, n), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareMove(), this._applyMove()) : (r$6.once(n$10.read, this._prepareMove, this._moveId), r$6.once(n$10.write, this._applyMove, this._moveId)));
					break;
			}
		}
		_onScroll() {
		...
		resolveStartPredicate(n, r) {
			...
			}), this.settings.sensorProcessingMode === P.Immediate ? (this._prepareStart(), this._applyStart()) : (r$6.once(n$10.read, this._prepareStart, this._startId), r$6.once(n$10.write, this._applyStart, this._startId)));
		}
		...
		stop() {
			...
			if (n.isEnded = !0, this._prepareStart(), this._applyStart(), this._startPhase = j.None, r$6.off(n$10.read, this._startId), r$6.off(n$10.write, this._startId), r$6.off(n$10.read, this._moveId), r$6.off(n$10.write, this._moveId), r$6.off(n$10.read, this._alignId), r$6.off(n$10.write, this._alignId), window.removeEventListener(`scroll`, this._onScroll, D), this._selectionChangeHandler &&= ((n.items[0]?.element?.ownerDocument ?? document).removeEventListener(`selectionchange`, this._selectionChangeHandler), null), this._pointerCaptureTarget && this._pointerCaptureId !== null) {
				try {
		...
		align(n = !1) {
			!this.drag || this.drag.isEnded || (n || this.settings.sensorProcessingMode === P.Immediate ? (this._prepareAlign(), this._applyAlign()) : (r$6.once(n$10.read, this._prepareAlign, this._alignId), r$6.once(n$10.write, this._applyAlign, this._alignId)));
		}
		getClientRect() {
	...
	const n$7 = e$8(), r$3 = e$8(), i$2 = {
		change: 0,
		drift: 0
	};
	function a$1(e, t, n, r, a, o, s) {
		let c = o, l = a;
		if (o > 0) {
	...
	function s$1(s, c) {
		...
		return function(t, c) {
			let f = e$8(s(c), n$7), p = e$8(c.item.clientRect, r$3), m = c.item.data, h = m.__containment__ || {
				drift: {
					x: 0,
					y: 0
				},
			...
			if (m.__containment__ ||= (h.trackDrift = typeof l == `function` ? l(c) : l, h), c.phase === `start`) return t.x &&= (a$1(p.left, p.right, f.left, f.right, 0, t.x, !1), i$2.change), t.y &&= (a$1(p.top, p.bottom, f.top, f.bottom, 0, t.y, !1), i$2.change), (u || d$2) && (h.startLeft = p.left + t.x, h.startTop = p.top + t.y, h.startRight = p.right + t.x, h.startBottom = p.bottom + t.y), t;
			if (u) {
				h.sensorX += t.x;
			...
			} else t.x &&= (a$1(p.left, p.right, f.left, f.right, h.drift.x, t.x, h.trackDrift), h.drift.x = i$2.drift, i$2.change);
			if (d$2) {
				h.sensorY += t.y;
			...
			} else t.y &&= (a$1(p.top, p.bottom, f.top, f.bottom, h.drift.y, t.y, h.trackDrift), h.drift.y = i$2.drift, i$2.change);
			return t;
		};
	}
	//#endregion
	...
	*/
	...
	function S(e) {
		...
		for (let n = 0; n < e.length; n++) {
			...
			S.position = `absolute`, S.left = `0px`, S.top = `0px`, S.margin = `0`, S.padding = `0`, S.boxSizing = `border-box`, S.pointerEvents = `none`, S.contain = `layout`, x.dataset.dragPreviewProxy = `true`, p[n] = i, t[n] = x, m[n] = a, h[n] = s, g[n] = c, _[n] = y, v[n] = b, f.has(i) || f.set(i, c$1(i));
		}
		for (let n = 0; n < e.length; n++) {
		...
		let k = (0, import_react.useRef)(void 0), A = r$1(() => {
			...
			let n = T.current.dragPreview, r = new R(w.current, {
				...
				elements(t) {
					...
					let i = S(r);
					i$1.add(t.draggable, r, i);
					let a = () => {
						let n = O.current || 0;
						if (n > 0) {
							for (let e of i) e.dataset.exiting = `true`;
							let r = !1, a = () => {
								r || (r = !0, clearTimeout(o), i$1.remove(t.draggable), setTimeout(() => {
									for (let e of i) e.remove();
								}, 0));
							}, o = setTimeout(a, n);
							i$1.startExiting(t.draggable, a);
						} else i$1.remove(t.draggable), setTimeout(() => {
							for (let e of i) e.remove();
						}, 0);
	...
	*/
	...
	//#endregion
	//#region examples/react/006-draggable-combined-modifiers/index.tsx
	...
	const DraggableCardMemo = (0, import_react.memo)(function DraggableCard() {
		...
		const draggableSettings = (0, import_react.useMemo)(() => ({
			...
			startPredicate: ({ event }) => {
				...
				return Math.sqrt(dx * dx + dy * dy) >= THRESHOLD ? true : void 0;
			},
		...
		const drag = r(C([pointerSensor], draggableSettings));
